...
<div class="max-w-xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit Permission</h1>
            <p class="text-xs text-slate-500 mt-0.5">Ubah nama hak akses: {{ $permission->name }}</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-6">
        <form action="{{ route('permission.update', $permission->id) }}" method="POST" class="m-0 space-y-6">
            @csrf
            @method('PUT')
