...
@section('title', 'Edit Transaksi POS')

@section('content')
<div class="max-w-6xl mx-auto space-y-6 animate-fade-in">
    <!-- Header -->
    <div class="flex items-center gap-4">
        <a href="{{ route('transaksi.show', $transaksi) }}" class="p-2 bg-white hover:bg-slate-50 text-slate-600 rounded-xl border border-slate-200 shadow-sm transition">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit Transaksi POS</h1>
            <p class="text-xs text-slate-550 mt-0.5 font-mono">{{ $transaksi->kode_transaksi }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('transaksi.update', $transaksi) }}" method="POST" class="m-0">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Pelanggan</label>
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('transaksi.show', $transaksi) }}" class="block w-full text-center py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-semibold text-xs transition">
                        Batal
                    </a>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    @if(old('obat_id'))
        ...
        @endforeach
    @elseif(isset($transaksi) && $transaksi->detailTransaksis->count() > 0)
        @foreach($transaksi->detailTransaksis as $detail)
            tambahBaris('{{ $detail->obat_id }}', '{{ $detail->jumlah }}');
        @endforeach
    @else
        tambahBaris();
    @endif
