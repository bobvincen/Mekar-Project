...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="{{ route('customer.create') }}" class="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white px-5 py-2.5 rounded-xl font-medium shadow-md hover:shadow-lg transition flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="py-4 px-6 whitespace-nowrap">
                        ...
                        @else
                            <span class="px-2.5 py-1 text-xs font-semibold bg-gray-100 text-gray-500 rounded-lg uppercase tracking-wider">
                                Belum Verifikasi
                            </span>
                        @endif
                    </td>
                    <td class="py-4 px-6 text-right whitespace-nowrap">
                        <a href="{{ route('customer.edit', $cust->id) }}" class="inline-block p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </a>
                        
                        <form action="{{ route('customer.destroy', $cust->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pelanggan ini?');">
                            @csrf
                            @method('DELETE')
