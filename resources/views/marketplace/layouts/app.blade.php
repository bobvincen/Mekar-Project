...
<head>
    ...
    <meta name="description"
        content="Mekar Pharmacy - Apotek online terpercaya. Dapatkan obat, vitamin, dan produk kesehatan berkualitas dengan mudah.">
...
@php
    ...
    $navIsProduct =
        ...
        request()->routeIs('marketplace.showProduct') ||
        request()->routeIs('marketplace.category');
@endphp

<body
    class="bg-[#f8fbff] text-slate-700 antialiased selection:bg-blue-200 selection:text-blue-900 relative min-h-screen flex flex-col"
    x-data="{
        scrolled: false,
        activeMenu: '{{ $navIsHome ? 'home' : ($navIsProduct ? 'products' : '') }}'
    }" @scroll.window="scrolled = (window.pageYOffset > 20)" x-init="if ('{{ $navIsHome }}' == '1') {
        setTimeout(() => {
    ...
    <div
        class="fixed inset-0 z-[-1] pointer-events-none overflow-hidden bg-[linear-gradient(180deg,#f8fbff_0%,#eef6ff_50%,#f8fbff_100%)]">
        <!-- Global Ambient Blobs -->
        <div
            class="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-blue-400/10 blur-[120px] animate-blob">
        </div>
        <div
            class="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-cyan-400/10 blur-[150px] animate-blob-slow">
        </div>
        <div
            class="absolute top-[40%] left-[60%] w-[400px] h-[400px] rounded-full bg-indigo-400/5 blur-[120px] animate-blob-delayed">
        </div>
        <!-- Grid Pattern -->
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b"
        :class="scrolled ? 'bg-white/90 backdrop-blur-xl border-slate-200/60 shadow-sm py-1.5' :
        ...
        <div class="max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-10">
            <div class="flex items-center justify-between h-12 gap-6">
                ...
                <a href="/" class="flex items-center gap-2 shrink-0 group">
                    <img src="/logo.png" alt="Mekar Pharmacy Logo"
                ...
                <div class="hidden lg:flex items-center gap-5">
                    <a href="/" class="text-[13px] font-semibold transition-colors duration-300 relative group"
                        ...
                        Beranda
                        <span
                            class="absolute -bottom-1 left-0 w-full h-[2px] bg-blue-600 rounded-full transition-transform duration-300 origin-left"
                    ...
                    <a href="/#konsultasi"
                        class="text-[13px] font-semibold transition-colors duration-300 relative group"
                ...
                <div class="hidden md:block flex-1 max-w-lg mx-auto">
                    <form action="/products" method="GET" class="relative group flex items-center">
                        ...
                        <button type="submit"
                            class="absolute right-1 top-1 bottom-1 w-8 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center transition-colors shadow-sm"
                            title="Cari">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    @guest
                        <a href="{{ route('login') }}"
                            class="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full text-[13px] font-semibold transition-all duration-300 shadow-md shadow-blue-600/20 hover:shadow-lg hover:-translate-y-0.5">
                    ...
                    @else
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.outside="open = false"
                                class="flex items-center gap-2 bg-white border border-slate-200/80 hover:border-blue-200 text-slate-700 hover:text-blue-600 pl-1.5 pr-2.5 py-1 rounded-full text-[13px] font-medium transition-all duration-300 shadow-sm focus:outline-none group">
                                <div
                                    class="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-[10px] font-bold">
                                    {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                                </div>
                                <span
                                    class="hidden sm:inline font-semibold">{{ explode(' ', Auth::user()->name)[0] }}</span>
                                <svg class="w-3 h-3 text-slate-400 group-hover:text-blue-500 transition-transform duration-200"
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                ...
                                <div class="px-4 py-2.5 border-b border-slate-50">
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Akun
                                        Saya</p>
                                    ...
                                    <span
                                        class="inline-flex items-center px-1.5 py-0.5 text-[9px] font-bold bg-blue-50 text-blue-600 rounded uppercase">
                                        {{ Auth::user()->role }}
                                    </span>
                                </div>
                                ...
                                <div class="py-1">
                                    @if (Auth::user()->can('Dashboard'))
                                        <a href="{{ Auth::user()->getDashboardUrl() }}"
                                            class="flex items-center gap-2.5 px-4 py-2 text-[13px] font-medium text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M7.5 9.75h9M3.375 19.5h17.25" />
                                            </svg>
                                            Dashboard Sistem
                                        </a>
                                    @endif
                                ...
                                <div class="px-2 pt-1.5 border-t border-slate-50 pb-0.5">
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="w-full flex items-center justify-center gap-2 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-[12px] font-bold hover:bg-red-100 transition-colors">
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <footer class="bg-slate-900 text-slate-300 border-t border-slate-800 relative overflow-hidden mt-auto z-10">
        ...
        <div
            class="absolute -top-[50%] -right-[10%] w-[50%] h-[150%] rounded-full bg-blue-900/30 blur-[100px] pointer-events-none">
        </div>
        ...
        <div class="max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-14 relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-6">
                ...
                <div class="md:col-span-4 pr-0 md:pr-10">
                    <a href="/" class="flex items-center gap-2 mb-4 group w-fit">
                        <div
                            class="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-lg shadow-white/10 group-hover:scale-105 transition-transform duration-300">
                    ...
                    <p class="text-slate-400 text-[13px] leading-relaxed mb-6 font-light">
                        Mekar Pharmacy adalah pelopor layanan kesehatan digital di Indonesia. Kami menghadirkan
                        kemudahan akses terhadap sediaan farmasi berkualitas.
                    </p>

                    <!-- Social Icons -->
                    <div class="flex gap-3">
                        <a href="#"
                            class="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-300 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all duration-300">
                ...
                <div class="md:col-span-3">
                    ...
                    <ul class="space-y-2.5 text-[13px] font-medium text-slate-400">
                        <li><a href="/" class="hover:text-white transition-colors flex items-center gap-2"><span
                                    class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Beranda</a></li>
                        <li><a href="/products"
                                class="hover:text-white transition-colors flex items-center gap-2"><span
                                    class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Semua Produk</a></li>
                        <li><a href="#kategori"
                                class="hover:text-white transition-colors flex items-center gap-2"><span
                                    class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Kategori Obat</a></li>
                    </ul>
                </div>
                ...
                <div class="md:col-span-5">
                    ...
                    <ul class="space-y-3 text-[13px] font-light text-slate-400">
                        <li class="flex items-start gap-2.5 group">
                            <div
                                class="w-7 h-7 rounded-md bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                    stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <a href="https://maps.app.goo.gl/XcgBAgXPczcEE5Cj9?g_st=iw" target="_blank"
                                rel="noopener noreferrer" class="pt-1 hover:text-white transition-colors">Apotek
                                Mekar, Jl. Ksatria, Gantiang Parak Gadang, Kec.Padang Tim, Kota Padang, Sumatera Barat
                                25133</a>
                        </li>
                        <li class="flex items-center gap-2.5 group">
                            ...
                            <span>mekarpharmacy@gmail.com</span>
                        </li>
                        <li class="flex items-center gap-2.5 group">
                            <div
                                class="w-7 h-7 rounded-md bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                    stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-2.896-1.596-5.273-3.974-6.869-6.869l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                                </svg>
                            </div>
            ...
            <div
                class="border-t border-white/10 mt-10 pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
    ...
    <button onclick="openKonsultasi('floating_button')"
        class="fixed bottom-6 right-6 z-[100] flex items-center justify-center w-14 h-14 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-[0_8px_20px_rgba(34,197,94,0.3)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_25px_rgba(34,197,94,0.4)] group"
        ...
        <span
            class="absolute right-full mr-4 bg-slate-800 text-white text-xs font-semibold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap">Konsultasi
            Apoteker</span>
    </button>
