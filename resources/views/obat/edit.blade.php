...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit Data Obat</h1>
            <p class="text-xs text-slate-500 mt-0.5">Ubah informasi detail untuk produk obat: {{ $obat->nama_obat }}</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-6">
        <form action="{{ route('obat.update', $obat->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return validasiObat()" class="m-0 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="stok" name="stok" value="{{ old('stok', $obat->stok) }}"
                ...
                <div>
                    ...
                    <input type="number" id="harga_jual" min="1" name="harga_jual" value="{{ old('harga_jual', $obat->harga_jual) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="date" id="tanggal_kadaluarsa" name="tanggal_kadaluarsa" value="{{ old('tanggal_kadaluarsa', $obat->tanggal_kadaluarsa) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    @if($obat->image_path)
                        <div id="current_image_container" class="mb-4 p-4 border border-slate-100 bg-slate-50/50 rounded-2xl flex items-center gap-4">
                            <img src="{{ $obat->image_url }}" alt="Current Image" class="w-16 h-16 rounded-xl object-cover border border-slate-200 shadow-sm">
                            <div>
                                ...
                                <button type="button" onclick="hapusGambarLama()" 
                                    class="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-xs font-bold transition flex items-center gap-1 border border-rose-100">
                                    Hapus Gambar
                                </button>
                            </div>
                        </div>
                    ...
                    <div id="editImagePreviewContainer" class="hidden mt-4">
                        <p class="text-xs text-slate-400 mb-2 font-semibold">Pratinjau Gambar Baru:</p>
                        <img id="editImagePreview" src="#" alt="Pratinjau Gambar Baru" class="max-h-44 rounded-xl border border-slate-100 shadow-sm object-cover">
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        ...
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-semibold text-slate-700 placeholder-slate-400">{{ old('deskripsi', $obat->deskripsi) }}</textarea>
                </div>
            </div>
...
<script>
    function supplierCombobox() {
        ...
        return {
            ...
            selectedId: "{{ old('supplier_id', $obat->supplier_id) }}",
