            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="input-modern relative flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden transition-all duration-300">
                        ...
                        <input type="password" name="password" placeholder="••••••••" required autocomplete="current-password" class="w-full bg-transparent border-none py-3.5 pl-1 pr-4 text-sm text-slate-800 placeholder-slate-400 focus:ring-0 font-medium">
                    </div>
                    @error('password')
                        <p class="text-red-500 text-xs mt-1.5 ml-1 font-bold">{{ $message }}</p>
                    @enderror
