...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        <div class="mb-4 sm:mb-0 flex items-center gap-3">
            <a href="{{ route('admin.transaksi-online.index') }}" class="p-2 rounded-lg border border-slate-200 bg-white text-slate-500 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div>
                <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Detail Pesanan: {{ $transaksi->kode_transaksi }}</h1>
                <p class="text-slate-500 text-sm mt-1">Dibuat pada {{ \Carbon\Carbon::parse($transaksi->tanggal_transaksi)->format('d M Y, H:i') }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            @endphp
            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold border shadow-sm {{ $statusColor }}">
    ...
    @if (session('success'))
        <div x-data="{ open: true }" x-show="open" class="mb-6 px-4 py-3 bg-emerald-50 border border-emerald-200 text-emerald-600 rounded-lg flex justify-between items-center shadow-sm">
            ...
            <button @click="open = false" class="text-emerald-400 hover:text-emerald-600"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
    @endif
    @if (session('error'))
        <div x-data="{ open: true }" x-show="open" class="mb-6 px-4 py-3 bg-red-50 border border-red-200 text-red-600 rounded-lg flex justify-between items-center shadow-sm">
            <span class="text-sm font-medium">{{ session('error') }}</span>
            <button @click="open = false" class="text-red-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    <svg class="w-6 h-6 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-5">
                ...
                <form action="{{ route('admin.transaksi-online.update-status', $transaksi->id) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    
                    <div>
                        <select name="status" class="w-full text-sm border-slate-200 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                            <option value="Menunggu Konfirmasi" {{ $transaksi->status == 'Menunggu Konfirmasi' ? 'selected' : '' }}>Menunggu Konfirmasi</option>
                            <option value="Diproses" {{ $transaksi->status == 'Diproses' ? 'selected' : '' }}>Diproses (Persiapan)</option>
                            <option value="Dikirim" {{ $transaksi->status == 'Dikirim' ? 'selected' : '' }}>Dikirim / Siap Diambil</option>
                            <option value="Selesai" {{ $transaksi->status == 'Selesai' ? 'selected' : '' }}>Selesai (Stok akan dikurangi)</option>
                            <option value="Dibatalkan" {{ $transaksi->status == 'Dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                        </select>
                        <p class="text-xs text-slate-500 mt-2">Pilih <b>Selesai</b> untuk mengonfirmasi bahwa pesanan telah diterima pelanggan dan stok obat akan dikurangi dari sistem.</p>
                    </div>
                    
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2">
            ...
            <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-5 space-y-4">
                ...
                <div>
                    ...
                    <div class="text-sm font-medium text-slate-800 flex items-center gap-1.5">
                        ...
                        @else
                            <svg class="w-4 h-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                        @endif
                ...
                <div>
                    ...
                    @if($transaksi->jarak)
                        <div class="text-xs text-slate-500 mt-1">Jarak: {{ number_format($transaksi->jarak, 2) }} KM</div>
                    @endif
                </div>
                @endif
            </div>
            ...
            <div class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-200 font-semibold text-slate-800 flex items-center gap-2">
                ...
                <div class="p-3 bg-slate-50 border-t border-slate-200">
                    <a href="https://maps.google.com/?q={{ $transaksi->latitude }},{{ $transaksi->longitude }}" target="_blank" class="text-sm text-blue-600 font-medium hover:underline flex items-center justify-center gap-1">
                        Buka di Google Maps
