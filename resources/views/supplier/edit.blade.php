...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit Data Supplier</h1>
            <p class="text-xs text-slate-500 mt-0.5">Ubah informasi detail rekanan pemasok: {{ $supplier->nama_supplier }}</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] p-6">
        <form action="{{ route('supplier.update', $supplier->id) }}" method="POST" class="m-0 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="kontak_pic" name="kontak_pic" value="{{ old('kontak_pic', $supplier->kontak_pic) }}"
                ...
                <div>
                    ...
                    <input type="text" id="kota" name="kota" value="{{ old('kota', $supplier->kota) }}"
                ...
                <div>
                    ...
                    <textarea id="alamat" name="alamat" rows="3" placeholder="Masukkan alamat lengkap distributor..." required
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-semibold text-slate-700 placeholder-slate-400 @error('alamat') border-rose-300 bg-rose-50/20 @enderror">{{ old('alamat', $supplier->alamat) }}</textarea>
                    @error('alamat')
                        <p class="text-rose-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" rows="2" placeholder="Keterangan tambahan jika ada..."
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-semibold text-slate-700 placeholder-slate-400 @error('keterangan') border-rose-300 bg-rose-50/20 @enderror">{{ old('keterangan', $supplier->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-rose-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
