...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">Manajemen Role</h1>
            <p class="text-sm text-slate-500 mt-1">Kelola peran hak akses serta pembagian wewenang pengguna sistem</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('role.create') }}" class="px-4 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white rounded-xl font-semibold text-xs shadow-md hover:shadow-lg transition inline-flex items-center gap-1.5">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left border-collapse table-auto">
                <thead class="bg-slate-50 text-slate-400 font-bold text-xs uppercase border-b border-slate-100 tracking-wider">
                    <tr>
                        <th class="py-4 px-6 w-56">Nama Role</th>
                        <th class="py-4 px-6">Permission Terkait</th>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-600 font-medium">
                    @forelse($roles as $role)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="py-4 px-6 font-bold text-slate-900 capitalize whitespace-nowrap">
                                {{ $role->name }}
                            </td>
                            <td class="py-4 px-6">
                                <div class="flex flex-wrap gap-1.5 py-1">
                                    @forelse($role->permissions as $perm)
                                        <span class="px-2.5 py-0.5 text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 rounded-lg">
                                            {{ $perm->name }}
                                        </span>
                                    @empty
                                        <span class="text-xs text-slate-400 italic font-normal">Tidak ada permission terkait</span>
                                    @endforelse
                                </div>
                            </td>
                            <td class="py-4 px-6">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('role.edit', $role->id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-xl transition" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-12 text-center text-slate-400 font-medium bg-slate-50/10">
