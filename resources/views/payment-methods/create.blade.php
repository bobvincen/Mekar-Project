...
@section('content')
<div class="max-w-3xl mx-auto space-y-8 animate-fade-in" x-data="{ type: 'bank_transfer' }">
    <!-- Header -->
    <div class="flex items-center gap-4 border-b border-slate-100 pb-5">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Tambah Metode Pembayaran</h1>
            <p class="text-xs text-slate-500 mt-0.5 font-semibold">Buat opsi pembayaran baru untuk pelanggan</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
        <form action="{{ route('admin.payment-methods.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Bank BCA, DANA, QRIS ShopeePay" required
            ...
            <div x-show="type === 'bank_transfer' || type === 'e_wallet'" class="grid grid-cols-1 sm:grid-cols-2 gap-6" x-transition>
                ...
                <div>
                    ...
                    <input type="text" name="account_owner" value="{{ old('account_owner') }}" placeholder="Contoh: Mekar Pharmacy"
                        ::required="type === 'bank_transfer' || type === 'e_wallet'"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-semibold text-slate-700 placeholder-slate-400">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Masukkan instruksi khusus atau catatan tambahan..."
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-semibold text-slate-700 placeholder-slate-400">{{ old('description') }}</textarea>
            </div>

            <!-- Upload Logo / QRIS Image -->
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2" x-text="type === 'qris' ? 'Gambar QRIS (Wajib upload)' : 'Logo Pembayaran (Opsional)'">Logo Pembayaran</label>
                ...
                <p class="text-[10px] text-slate-400 mt-1.5 font-medium">Unggah gambar dengan format JPG, JPEG, PNG, atau WEBP. Maksimal 2MB.</p>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <label for="is_active" class="text-xs font-bold text-slate-655 cursor-pointer">Aktifkan Metode Pembayaran ini Langsung</label>
            </div>
